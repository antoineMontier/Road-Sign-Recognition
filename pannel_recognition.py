...
def filter_c(contours, image):
    ...
    for cont in contours:
        ...
        if(c_area > 12**2 and c_area > image_area*.0005 and .33 < ratio < 3):
            n_list.append(cont)
    return n_list

model = load_model('shape-recognizerv2-5eh.h5')
...
def get_pannels(contours, threshold=.8):
    ...
    for r in range(len(res)):
        for k in range(r + 1, len(res)):
            if common_area((res[r].min_x, res[r].max_x, res[r].min_y, res[r].max_y), (res[k].min_x, res[k].max_x, res[k].min_y, res[k].max_y)) < .5:
                n_res.append(res[r])
...
for i in range(1, 172):
    ...
    pannels = get_pannels(contours, 8)
